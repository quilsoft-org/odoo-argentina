...
class ResCompany(models.Model):
    ...
    def arba_connect(self):
        ...
        try:
            ...
            ws = None
            _logger.exception("ARBA exception: {}".format(e))

        return ws

    def get_agip_data(self, partner, date):
        raise UserError(_(
            'Falta configuración de credenciales para consulta de '
            'Alícuotas de AGIP'))
    ...
    def get_arba_data(self, partner, from_date, to_date):
        ...
        _logger.info('Getting ARBA data for cuit {} from date {} to date {}'.format(cuit, from_date, to_date))
    ...
    def get_cordoba_data(self, partner, date):
        ...
        if code == 3:
            ...
            alicuota_percepcion = 0.000
            alicuota_retencion = 0.00
        elif code != 0:
    ...
    def _process_message_error(self, ws):
        ...
        _logger.info(_('No se pudo conectar a ARBA: %s - %s') % (ws.CodigoError, message))
