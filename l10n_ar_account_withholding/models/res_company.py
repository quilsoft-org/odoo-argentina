...
class ResCompany(models.Model):
    ...
    def _localization_use_withholdings(self):
        ...
        return (
            ...
            else super()._localization_use_withholdings()
        )
    ...
    def arba_connect(self):
        ...
        if not self.arba_cit:
            raise UserError(
                _("You must configure ARBA CIT on company %s") % (self.name)
            )
        ws = IIBB()
        ...
        _logger.info(
            'Connection getted to ARBA with url "%s" and CUIT %s' % (arba_url, cuit)
        )
        return ws
    ...
    def get_arba_data(self, partner, from_date, to_date):
        ...
        ws.ConsultarContribuyentes(
            from_date.strftime("%Y%m%d"), to_date.strftime("%Y%m%d"), cuit
        )
        ...
        data = {
            ...
            "alicuota_percepcion": ws.AlicuotaPercepcion
            and float(ws.AlicuotaPercepcion.replace(",", ".")),
            "alicuota_retencion": ws.AlicuotaRetencion
            and float(ws.AlicuotaRetencion.replace(",", ".")),
    ...
    def get_cordoba_data(self, partner, date):
        ...
        if r.status_code != 200:
            raise UserError(
                "Error al contactar rentascordoba.gob.ar. "
        ...
        if code == 3:
            ...
            if dict_alic.get("CRD_PAR_CODIGO") != "NUE_INS":
                ...
                from_date_date = fields.Date.from_string(
                    dict_alic.get("CRD_FECHA_INICIO")
                )
