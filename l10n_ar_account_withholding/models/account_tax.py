...
class AccountTax(models.Model):
    ...
    withholding_type = fields.Selection(
        ...
        ondelete={"partner_tax": "cascade", "tabla_ganancias": "cascade"},
    )
    ...
    def check_partner_tax_tag(self):
        recs = self.filtered(
            lambda x: (
                (
                    x.type_tax_use in ["sale", "purchase"]
                    and x.amount_type == "partner_tax"
                )
                or (
                    x.type_tax_use in ["customer", "supplier"]
                    and x.withholding_type == "partner_tax"
                )
            )
            and not x.invoice_repartition_line_ids.mapped("tag_ids")
        )
        if recs:
            raise UserError(
                _(
                    ...
                    " el partner. Revise los impuestos con id: %s"
                )
                % recs.ids
            )
    ...
    def get_period_payments_domain(self, payment_group):
        previos_payment_groups_domain, previos_payments_domain = super(
            AccountTax, self
        ).get_period_payments_domain(payment_group)
        if (
            self.withholding_type == "tabla_ganancias"
            and payment_group.retencion_ganancias == "nro_regimen"
        ...
        return (previos_payment_groups_domain, previos_payments_domain)
        ...
        if (
            ...
            if arba_tag and arba_tag.id in invoice_tags.ids:
                ...
                if not agip_data["numero_comprobante"]:
                    agip_data["numero_comprobante"] = "Alícuota no inscripto"
                    agip_data[
                        "alicuota_retencion"
