...
class AccountTax(models.Model):
    ...
    withholding_type = fields.Selection(
        selection_add=([
            ...
            ('partner_tax', 'Alícuota en el Partner'),
        ]), ondelete={'partner_tax': 'set default', 'tabla_ganancias': 'set default'}
    )
    ...
    def get_withholding_vals(self, payment_group):
        ...
        _logger.info('tax {} of withholding type {}'.format(self.name, self.withholding_type))
        ...
        _logger.info('parent withholding vals {}'.format(vals))
        ...
        _logger.info('returning withholding vals {}'.format(vals))
        return vals
    ...
    def get_partner_alicuot(self, partner, date):
        self.ensure_one()
        """ Se comentan las líneas de computo de alicuotas de AGIP porque no existe una conexión con su correspondiente WS """
        ...
        alicuot = partner.arba_alicuot_ids.search([
            ...
            # no es necesario la busqueda por rango de fechas de impuestos
            # '|',
            # ('from_date', '=', False),
            # ('from_date', '<=', date),
            # '|',
