...
class L10nArCustomerPortal(CustomerPortal):
    ...
    def details_form_validate(self, data):
        ...
        write_error, write_message = request.env["res.partner"].try_write_commercial(
            data
        )
        if write_error:
    ...
    def account(self, redirect=None, **post):
        ...
        identification_types = (
            request.env["l10n_latam.identification.type"].sudo().search([])
        )
        afip_responsibilities = (
            request.env["l10n_ar.afip.responsibility.type"].sudo().search([])
        )
        uid = request.session.uid
        partner = (
            request.env["res.users"].browse(uid).partner_id
            if uid
